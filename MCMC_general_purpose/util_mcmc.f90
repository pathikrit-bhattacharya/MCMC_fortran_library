...
contains 
   ...
   ! Use a standard normal multivariate and convert to a random variable from a
   ! multivariate normal distribution with mean mu and standard deviation std
   ...
   ! Mahalanobis distance for vector x
   arg   = -0.5*dot_product(matmul(cinv,x-mu),(x-mu))
   N     = dble(size(x))
   ! Find the determinant of std
   det_s = det_sym(std)
   ...
   ! Correlated uniform random numbers for proposal generation
   ! Based on the recipe in http://comisef.wikidot.com/tutorial:correlateduniformvariates
   ! We produce Bravais-Pearson correlation from a pre-specified Spearman correlation
   ! Then use the normal distribution to map the correlation onto a uniform distribution
 ...
 !!! means and std-s according to a predefined probability, mix_factor, to choose from the first and
 !!! a probability 1-mix_factor to draw from the second.
 implicit none
