...
subroutine combined_model(p_acc)
	  ...
	  do ind = 1,n_dat_files
		...
		do i = 1, Nav
			call getparams(p_acc(:,collect(i)),p_waste)
			call fmodel(p_acc(:,collect(i)), tau, slipv, sliprate, state, x_f, x_lp, t, v_lp, Nrange, ind)
...
subroutine fmodel(rnstry, tau, slipv, sliprate, state, x_f, x_lp, t, v_lp, Nrange, runind)
...
!
	implicit none
	  ...
	  integer, intent(in)                :: runind, Nrange
	  real(8)                            :: aa,bb,dc,Vc,stiffk,vo,c
